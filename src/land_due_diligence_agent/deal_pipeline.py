...
def run_local_deal_pipeline(
    ...
    *,
    ...
    logger = configure_logging(settings.log_level, deal_paths.output_run_dir / "run.log")
    ...
    logger.info("Text extraction output: %s", deal_paths.text_run_dir)
    logger.info("Metadata output: %s", deal_paths.metadata_run_dir)
    logger.info("Report output: %s", deal_paths.output_run_dir)
    try:
        ...
        for file_path in discovered_files:
            ...
            try:
                ...
                manifest_entry.extracted_text_path = str(text_path)
                manifest_entry.structured_text_path = str(json_path)
                ...
                manifest_entry.errors.append(f"{type(exc).__name__}: {exc}")
        ...
        logger.info("Markdown review: %s", result.review_report_path)
...
def _write_run_artifacts(result: DealRunResult) -> None:
    manifest_json_path = result.deal_paths.metadata_run_dir / "deal_manifest.json"
    manifest_csv_path = result.deal_paths.metadata_run_dir / "deal_manifest.csv"
    issue_registry_path = result.deal_paths.metadata_run_dir / "issue_registry.json"
    run_summary_path = result.deal_paths.metadata_run_dir / "run_summary.json"
    latest_run_path = result.deal_paths.metadata_dir / "latest_run.json"
    review_report_path = result.deal_paths.output_run_dir / "first_pass_due_diligence_review.md"
    ...
    result.latest_run_path = str(latest_run_path)
    ...
    summary_payload = {
        ...
        "source_drop_dir": str(result.deal_paths.source_drop_dir),
        "text_extraction_dir": str(result.deal_paths.text_run_dir),
        "metadata_dir": str(result.deal_paths.metadata_run_dir),
        "report_dir": str(result.deal_paths.output_run_dir),
    ...
    write_json(latest_run_path, summary_payload)
    write_review_markdown(review_report_path, result)