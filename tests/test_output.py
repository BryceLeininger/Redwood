...
class OutputWriterTests(unittest.TestCase):
    def test_writes_expected_markdown_files(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("Primary Deal Driver", synthesis_content)
            self.assertIn("Primary driver:", synthesis_content)
            self.assertIn("What must be true to close", synthesis_content)
            ...
            self.assertIn("Primary driver:", ic_content)
            self.assertIn("What has to be true", ic_content)
            self.assertIn("What must be true to start grading", ic_content)
            self.assertIn("What must be true for vertical start", ic_content)
