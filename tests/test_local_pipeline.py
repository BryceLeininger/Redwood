...
class LocalDealPipelineTests(unittest.TestCase):
    def test_pipeline_writes_stable_manifest_registry_and_docx_report(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("Deal Stage", review_text)
            self.assertIn("Top 3 Real Risks", review_text)
            self.assertIn("What Is Truly Gating Vs Routine", review_text)
            self.assertIn("What Is Missing But Actually Material", review_text)
            self.assertIn("Overall Package Read", review_text)
            self.assertEqual(paragraph_styles["BOTTOM LINE"], "Heading 1")
            self.assertEqual(paragraph_styles["Deal Stage"], "Heading 1")
            self.assertEqual(paragraph_styles["Top 3 Real Risks"], "Heading 1")
            self.assertEqual(paragraph_styles["What Is Truly Gating Vs Routine"], "Heading 1")
            self.assertEqual(paragraph_styles["What Is Missing But Actually Material"], "Heading 1")
            self.assertEqual(paragraph_styles["Overall Package Read"], "Heading 1")
            self.assertEqual(paragraph_styles["Gating"], "Heading 2")
            self.assertEqual(paragraph_styles["Routine"], "Heading 2")
            ...
            self.assertNotIn("What A Weak Acquisitions Person Would Miss", review_text)
            self.assertNotIn("Deal Overview", review_text)
            self.assertNotIn("DEAL IMPACT SUMMARY", review_text)
            self.assertNotIn("UNDERWRITE CONFIDENCE", review_text)
            self.assertIn("Decision: Needs Targeted Confirmation.", review_text)
            self.assertIn("approved horizontal land", review_text)
