"""Desktop launcher for the Residential Land Due Diligence intake panel."""
...
def _start_server() -> None:
    ...
    log_path = _repo_root() / "generated_agents" / "residential_land_due_diligence_panel.log"
    ...
    subprocess.Popen(
        [
            ...
            "-m",
            "agent_factory.residential_land_due_diligence_panel_server",
            "--host",
            HOST,
            "--port",
            str(PORT),
...
def _stop_server_on_port() -> None:
    ...
    command = (
        "$conn = Get-NetTCPConnection -LocalPort 8786 -State Listen -ErrorAction SilentlyContinue | "
