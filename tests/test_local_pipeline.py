...
class LocalDealPipelineTests(unittest.TestCase):
    def test_pipeline_writes_stable_manifest_registry_and_docx_report(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("Sanity Check / Corrections", review_text)
            self.assertIn("Controlling Facts", review_text)
            self.assertIn("Real Risk Classification", review_text)
            self.assertIn("Critical Path / Gating Chain", review_text)
            self.assertIn("Investment Decision", review_text)
            self.assertIn("What A Weak Acquisitions Person Would Miss", review_text)
            ...
            self.assertEqual(paragraph_styles["Sanity Check / Corrections"], "Heading 1")
            self.assertEqual(paragraph_styles["Controlling Facts"], "Heading 1")
            self.assertEqual(paragraph_styles["Real Risk Classification"], "Heading 1")
            self.assertEqual(paragraph_styles["Critical Path / Gating Chain"], "Heading 1")
            self.assertEqual(paragraph_styles["Investment Decision"], "Heading 1")
            self.assertEqual(paragraph_styles["What A Weak Acquisitions Person Would Miss"], "Heading 1")
