...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Run the local land due diligence workflow against one deal folder.",
    )
    target_group = parser.add_mutually_exclusive_group(required=True)
    target_group.add_argument(
        "--deal",
        help="Deal folder name under DEFAULT_DEALS_ROOT or --deals-root.",
    )
    target_group.add_argument(
        "--deal-folder",
        help="Explicit path to a deal folder containing 00_Source_Drop.",
    )
    target_group.add_argument(
        "--input-folder",
        help="Legacy prototype input-folder mode for the earlier experimental pipeline.",
    )
    parser.add_argument(
        "--deals-root",
        help="Base directory containing deal folders. Defaults to DEFAULT_DEALS_ROOT or ~/Desktop/Agent_Diligence/Deals.",
    )
    parser.add_argument(
        "--output-folder",
        help="Legacy base output folder. Defaults to DEFAULT_OUTPUT_DIR from .env or data/output.",
    )
    parser.add_argument("--deal-name", help="Optional deal name override for headings and output folders.")
    parser.add_argument(
        "--mode",
        choices=("fast", "full"),
        default="fast",
        help="Legacy analysis depth used only with --input-folder.",
    )
    parser.add_argument(
        "--llm-provider",
...
def main(argv: list[str] | None = None) -> int:
    """Run either the local deal workflow or the legacy prototype workflow."""
    ...
    if args.deal or args.deal_folder:
        return _run_local_mode(args, settings)
    return _run_legacy_mode(args, settings)


def _run_local_mode(args: argparse.Namespace, settings: Settings) -> int:
    deal_folder = _resolve_deal_folder(args, settings)
    try:
        _, exit_code = run_local_deal_pipeline(
            deal_folder,
            settings=settings,
...
def _resolve_deal_folder(args: argparse.Namespace, settings: Settings) -> Path:
    if args.deal_folder:
        return Path(args.deal_folder).expanduser().resolve()
    deals_root = Path(args.deals_root or settings.default_deals_root).expanduser().resolve()
    return (deals_root / args.deal).resolve()


def _run_legacy_mode(args: argparse.Namespace, settings: Settings) -> int:
